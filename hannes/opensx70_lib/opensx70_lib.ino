...
void BlinkISO3() { //read the default ISO and blink once for SX70 and twice for 600BW and triple for 600 - on ISO change.
    Serial.println("Blink Grren on ISO change");
    //savedISO = ReadISO();
    turnLedsOff();
      ...
      if((switch2 == 1) && (switch1 == 1)){ //Save ISO
        savedISO = ReadISO(); //read the savedISO from the EEPROM
          ...
          if(savedISO != _selectedISO){
            #if SIMPLEDEBUG
              Serial.print("ISO has changed, previos saved ISO(from the EEPROM): ");
              Serial.println(savedISO);
            ...
            BlinkISO2(); //Blink ISO and load saved ISO from EEPROM
        }
      }
    }
}
  ...
  if((switch2 != 1) && (switch1 != 1)){ //Save ISO
    if(nowDongle != 0){ //Donngle is present
        savedISO = ReadISO(); //read the savedISO from the EEPROM
          ...
          if(activeISO != _selectedISO){ //doesnt work that way
            #if SIMPLEDEBUG
            Serial.print("ISO has changed, savedISO:");
            Serial.println(savedISO);
            Serial.print("selectedISO: ");
            Serial.println(_selectedISO);
            #endif
            activeISO = _selectedISO;
            BlinkISO3(); //Blink Green on ISO select - only check no save
        }
    }
  }
}
...
void checkFilmCount(){
  //checkedcount = 0;//maybe obsolete
  //if (checkedcount == 0){
    //checkedcount=1;
    if ((currentPicture == 8) || currentPicture == 9){
        #if SIMPLEDEBUG
          Serial.println("Two Frames left");
...
void ispackEmpty(){
  ...
  if ((digitalRead(PIN_S8) == LOW && digitalRead(PIN_S9) == HIGH) && (currentPicture >= 8))
  {
    if (firstRun==0){
      ...
      #if SIMPLEDEBUG
          Serial.println("STATE2: PACK IS EMPTY - S9 Closed");
...
void positionB(){
    ...
    if  ((switch2 == 1) && (switch1 == 1)){ //LightmeterHelper Deactivation
...
void positionT(){
  ...
  if  ((switch2 == 1) && (switch1 == 1)){//LightmeterHelper Activation
...
void manualExposure(){
  ...
  if ((selector >= 0) && (selector < 11)) //Manual Exposure
  {
    LightMeterHelper(1);
...
void Auto600Exposure(){
  //Auto600
  if (((ShutterSpeed[selector]) == AUTO600)) //AUTO600 WHEELEEL
  {
    LightMeterHelper(0);
    if ((sw_S1.clicks == -1) || (sw_S1.clicks > 0)) 
    {
      switch2Function(2); //Switch 2 Function on AUTOEXPOSURE 600
      sw_S1.Reset();
...
void LightMeterHelper(byte ExposureType){
    int helperstatus = openSX70.getLIGHTMETER_HELPER()==true;
    if(helperstatus==true){
      if(metercount==3){ //Lightmeter only on every 5th Cycle of Loop
