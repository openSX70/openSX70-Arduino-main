void loop() {
  ...
  long exposure = frequencyCounter() + ShutterConstant;

  Serial.print ("Exposure time Ratio : ");
  ...
  Serial.println (ShutterSpeedDelay/exposure);
  
    if ((selector >= 0) && (selector <= 10))
  {
    if ((ShutterSpeed[selector]/exposure) == 1)
      digitalWrite(led1, HIGH);
  else
      digitalWrite(led1, LOW);
  
    /*
    else if (ShutterSpeed[selector]/exposure) < 2
    {
      if   ((exposure > ShutterSpeed[selector + 1]) && (exposure < ShutterSpeed[selector - 1]))
        digitalWrite(led1, HIGH);
      else
        digitalWrite(led1, LOW);
    }
