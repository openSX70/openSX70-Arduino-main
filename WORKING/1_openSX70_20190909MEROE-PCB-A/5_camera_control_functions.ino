...
//  while (digitalRead(S3) != HIGH)            //waiting for S3 to OPEN
  ...
  while (digitalRead(S1) == !S1Logic)
//  while (DebouncedRead(S1) == !S1Logic)
    ;

  if (digitalRead(S1) == S1Logic)
  {
    shutterCLOSE ();
  }
...
/*
 int debounce = 5;
  ...
  while ((digitalRead(S5) != LOW) && ((millis() - imillis) > debounce))  //While S5 open keep the motor running and lower the mirror
    ;
*/
//delay (5);
