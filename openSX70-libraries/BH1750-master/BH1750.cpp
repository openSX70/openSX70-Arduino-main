 ...
 */
bool BH1750::begin(Mode mode, byte sensitivity)
{

	// I2C is expected to be initialized outside this library

	// Configure sensor in specified mode
	return configure(mode, sensitivity);

}

bool BH1750::i2cSetMeasTime(byte val)
{
	if (!i2cwrite(BH1750_I2CADDR, 0x40 | ((val & 0xE0) >> 5)))
		return false;

	return i2cwrite(BH1750_I2CADDR, 0x60 | (val & 0x1F));
}

bool BH1750::i2cwrite(int i2caddr, uint8_t val)
{
 ...
 * @param mode Measurement mode
 */
bool BH1750::configure(Mode mode, byte sensitivity)
{

	if (!i2cSetMeasTime(MeasTimeRegvalFromSensi(sensitivity)))
		return false;

	if (i2cwrite(BH1750_I2CADDR, mode))
	{
		BH1750_MODE = mode;
		BH1750_sensitivity = sensitivity;
		
	} 
	else 
		return false;

	//Initial wait only in the continuous modes. One time modes will wait in readlightLevel
	if (BH1750_MODE != BH1750::ONE_TIME_LOW_RES_MODE && BH1750_MODE != BH1750::ONE_TIME_HIGH_RES_MODE && BH1750_MODE != BH1750::ONE_TIME_HIGH_RES_MODE_2)
		WaitForMeasurement(true);

}


void BH1750::WaitForMeasurement(bool maxWait)
{
	if (BH1750_MODE == BH1750::ONE_TIME_LOW_RES_MODE || BH1750_MODE == BH1750::CONTINUOUS_LOW_RES_MODE)
	{
		maxWait ? _delay_ms(24 * BH1750_sensitivity) : _delay_ms(16 * BH1750_sensitivity);
	} else
	{
		maxWait ? _delay_ms(180 * BH1750_sensitivity) : _delay_ms(120 * BH1750_sensitivity);
	}
}
...
uint16_t BH1750::readLightLevel(bool maxWait)
{
	...
	// A continuous mode measurement can be read immediately after the initial wait

	//One time modes need to set mode command again
	switch (BH1750_MODE)
	{
	...
	case BH1750::ONE_TIME_HIGH_RES_MODE_2:
		if (!i2cSetMeasTime(MeasTimeRegvalFromSensi(BH1750_sensitivity)))
			return 0;

		// Send mode to sensor
		if (!i2cwrite(BH1750_I2CADDR, BH1750_MODE))
			return 0;

		WaitForMeasurement(maxWait);
		break;
	default:
		break;
	}
